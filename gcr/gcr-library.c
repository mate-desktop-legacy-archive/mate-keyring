...
_gcr_initialize (void)
{
	...
	if (g_once_init_enter (&gcr_initialized)) {
		...
		trust_store_uri = g_strdup ("pkcs11:library-manufacturer=MATE%20Keyring;serial=1:XDG:DEFAULT");

		trust_lookup_uris = g_new0 (gchar*, 3);
		trust_lookup_uris[0] = g_strdup ("pkcs11:library-manufacturer=MATE%20Keyring;serial=1:ROOTS:DEFAULT");
		trust_lookup_uris[1] = g_strdup ("pkcs11:library-manufacturer=MATE%20Keyring;serial=1:XDG:DEFAULT");
