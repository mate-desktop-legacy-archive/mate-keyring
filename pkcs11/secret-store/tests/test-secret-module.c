...
test_secret_collection_validate (GkmSecretCollection *collection, GkmSecretData *sdata)
{
...
#if 0
	/* "Invalid item type" */
	g_assert_cmpint (item->type, ==, MATE_KEYRING_ITEM_GENERIC_SECRET);
#endif
	/* "Missing secret" */
	...
	ac = (MateKeyringAccessControl*)item->acl->data;
	/* "Invalid ACL" */
	...
	g_assert_cmpint (ac->types_allowed, ==, (MATE_KEYRING_ACCESS_READ | MATE_KEYRING_ACCESS_WRITE | MATE_KEYRING_ACCESS_REMOVE));
#endif
