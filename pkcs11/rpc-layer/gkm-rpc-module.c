...
call_run (CallState *cs)
{
	...
	if (cs->resp->call_id == GKM_RPC_CALL_ERROR) {
		...
		if (!gkm_rpc_message_read_ulong (cs->resp, &ckerr)) {
			warning (("invalid error response from mate-keyring-daemon: too short"));
			return CKR_DEVICE_ERROR;
		}

		if (ckerr <= CKR_OK) {
			warning (("invalid error response from mate-keyring-daemon: bad error code"));
			return CKR_DEVICE_ERROR;
		}
	...
	if (cs->req->call_id != cs->resp->call_id) {
		warning (("invalid response from mate-keyring-daemon: call mismatch"));
		return CKR_DEVICE_ERROR;
	}
	...
	if (cs->call_status == CALL_PARSE && cs->req && cs->resp) {
		...
		if (ret == CKR_OK) {
			...
			if (gkm_rpc_message_buffer_error (cs->resp)) {
				warning (("invalid response from mate-keyring-daemon: bad argument data"));
...
rpc_C_Initialize (CK_VOID_PTR init_args)
{
	...
	pthread_mutex_lock (&init_mutex);
		...
		if (pkcs11_socket_path == NULL) {
			path = getenv ("MATE_KEYRING_CONTROL");
			if (path && path[0]) {
