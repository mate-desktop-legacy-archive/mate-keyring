...
struct _GkmMate2PublicKey {
	GkmPublicXsaKey parent;
};

static void gkm_mate2_public_key_serializable (GkmSerializableIface *iface);

G_DEFINE_TYPE_EXTENDED (GkmMate2PublicKey, gkm_mate2_public_key, GKM_TYPE_PUBLIC_XSA_KEY, 0,
               G_IMPLEMENT_INTERFACE (GKM_TYPE_SERIALIZABLE, gkm_mate2_public_key_serializable));
...
static void
gkm_mate2_public_key_init (GkmMate2PublicKey *self)
{

}

static void
gkm_mate2_public_key_finalize (GObject *obj)
{
	/* GkmMate2PublicKey *self = GKM_MATE2_PUBLIC_KEY (obj); */
	G_OBJECT_CLASS (gkm_mate2_public_key_parent_class)->finalize (obj);
}

static void
gkm_mate2_public_key_set_property (GObject *obj, guint prop_id, const GValue *value,
                                  GParamSpec *pspec)
{
	switch (prop_id) {
	default:
...
static void
gkm_mate2_public_key_get_property (GObject *obj, guint prop_id, GValue *value,
                                  GParamSpec *pspec)
{
	switch (prop_id) {
	default:
...
static void
gkm_mate2_public_key_class_init (GkmMate2PublicKeyClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->finalize = gkm_mate2_public_key_finalize;
	gobject_class->set_property = gkm_mate2_public_key_set_property;
	gobject_class->get_property = gkm_mate2_public_key_get_property;
}
...
static void
gkm_mate2_public_key_serializable (GkmSerializableIface *iface)
{
	iface->extension = ".pub";
	iface->load = gkm_mate2_public_key_real_load;
	iface->save = gkm_mate2_public_key_real_save;
}
...
GkmFactory*
gkm_mate2_public_key_get_factory (void)
{
