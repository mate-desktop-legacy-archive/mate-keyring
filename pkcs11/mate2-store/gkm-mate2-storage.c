...
G_DEFINE_TYPE (GkmMate2Storage, gkm_mate2_storage, GKM_TYPE_STORE);
...
type_from_extension (const gchar *extension)
{
	...
	if (strcmp (extension, ".pkcs8") == 0)
		return GKM_TYPE_MATE2_PRIVATE_KEY;
	else if (strcmp (extension, ".pub") == 0)
		return GKM_TYPE_MATE2_PUBLIC_KEY;
...
static gboolean
check_object_hash (GkmMate2Storage *self, const gchar *identifier, const guchar *data, gsize n_data)
{
	gconstpointer value;
	GkmDataResult res;
	gboolean result;
	...
	res = gkm_mate2_file_read_value (self->file, identifier, CKA_MATE_INTERNAL_SHA1, &value, &n_value);
...
static void
data_file_entry_removed (GkmMate2File *store, const gchar *identifier, GkmMate2Storage *self)
{
	GkmObject *object;
...
static CK_RV
refresh_with_login (GkmMate2Storage *self, GkmSecret *login)
{
	GkmDataResult res;
	struct stat sb;
	CK_RV rv;
	...
	g_assert (GKM_MATE2_STORAGE (self));
	...
	res = gkm_mate2_file_read_fd (self->file, fd, login);
	switch (res) {
	case GKM_DATA_FAILURE:
...
static GObject*
gkm_mate2_storage_constructor (GType type, guint n_props, GObjectConstructParam *props)
{
	GkmMate2Storage *self = GKM_MATE2_STORAGE (G_OBJECT_CLASS (gkm_mate2_storage_parent_class)->constructor(type, n_props, props));
...
static void
gkm_mate2_storage_init (GkmMate2Storage *self)
{
	self->file = gkm_mate2_file_new ();
...
static void
gkm_mate2_storage_dispose (GObject *obj)
{
	GkmMate2Storage *self = GKM_MATE2_STORAGE (obj);
	...
	G_OBJECT_CLASS (gkm_mate2_storage_parent_class)->dispose (obj);
}

static void
gkm_mate2_storage_finalize (GObject *obj)
{
	GkmMate2Storage *self = GKM_MATE2_STORAGE (obj);
	...
	G_OBJECT_CLASS (gkm_mate2_storage_parent_class)->finalize (obj);
}

static void
gkm_mate2_storage_set_property (GObject *obj, guint prop_id, const GValue *value,
                           GParamSpec *pspec)
{
	GkmMate2Storage *self = GKM_MATE2_STORAGE (obj);
...
static void
gkm_mate2_storage_get_property (GObject *obj, guint prop_id, GValue *value,
                               GParamSpec *pspec)
{
	GkmMate2Storage *self = GKM_MATE2_STORAGE (obj);
	...
	switch (prop_id) {
	case PROP_DIRECTORY:
		g_value_set_string (value, gkm_mate2_storage_get_directory (self));
		break;
	case PROP_MODULE:
		g_value_set_object (value, self->module);
		break;
	case PROP_MANAGER:
		g_value_set_object (value, gkm_mate2_storage_get_manager (self));
		break;
	case PROP_LOGIN:
		g_value_set_object (value, gkm_mate2_storage_get_login (self));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (obj, prop_id, pspec);
		break;
...
static void
gkm_mate2_storage_class_init (GkmMate2StorageClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	GkmStoreClass *store_class = GKM_STORE_CLASS (klass);

	gobject_class->constructor = gkm_mate2_storage_constructor;
	gobject_class->dispose = gkm_mate2_storage_dispose;
	gobject_class->finalize = gkm_mate2_storage_finalize;
	gobject_class->set_property = gkm_mate2_storage_set_property;
	gobject_class->get_property = gkm_mate2_storage_get_property;

	store_class->read_value = gkm_mate2_storage_real_read_value;
	store_class->write_value = gkm_mate2_storage_real_write_value;
...
CK_RV
gkm_mate2_storage_refresh (GkmMate2Storage *self)
{
	g_return_val_if_fail (GKM_MATE2_STORAGE (self), CKR_GENERAL_ERROR);
...
CK_RV
gkm_mate2_storage_unlock (GkmMate2Storage *self, GkmSecret *login)
{
	CK_RV rv;
...
CK_RV
gkm_mate2_storage_lock (GkmMate2Storage *self)
{
	GkmSecret *prev;
	CK_RV rv;
...
GkmManager*
gkm_mate2_storage_get_manager (GkmMate2Storage *self)
{
	g_return_val_if_fail (GKM_IS_MATE2_STORAGE (self), NULL);
	return self->manager;
}

const gchar*
gkm_mate2_storage_get_directory (GkmMate2Storage *self)
{
	g_return_val_if_fail (GKM_IS_MATE2_STORAGE (self), NULL);
...
GkmSecret*
gkm_mate2_storage_get_login (GkmMate2Storage *self)
{
	g_return_val_if_fail (GKM_IS_MATE2_STORAGE (self), NULL);
	return self->login;
}
...
gulong
gkm_mate2_storage_token_flags (GkmMate2Storage *self)
{
	gulong flags = 0;
	CK_RV rv;
	...
	if (gkm_mate2_file_have_section (self->file, GKM_MATE2_FILE_SECTION_PRIVATE))
