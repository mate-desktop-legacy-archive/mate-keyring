...
static void gkm_mate2_private_key_serializable (GkmSerializableIface *iface);

G_DEFINE_TYPE_EXTENDED (GkmMate2PrivateKey, gkm_mate2_private_key, GKM_TYPE_PRIVATE_XSA_KEY, 0,
               G_IMPLEMENT_INTERFACE (GKM_TYPE_SERIALIZABLE, gkm_mate2_private_key_serializable));
...
static CK_RV
gkm_mate2_private_key_real_get_attribute (GkmObject *base, GkmSession *session, CK_ATTRIBUTE_PTR attr)
{
	switch (attr->type) {
	...
	return GKM_OBJECT_CLASS (gkm_mate2_private_key_parent_class)->get_attribute (base, session, attr);
}

static GkmSexp*
gkm_mate2_private_key_real_acquire_crypto_sexp (GkmSexpKey *base, GkmSession *unused)
{
...
static void
gkm_mate2_private_key_init (GkmMate2PrivateKey *self)
{

}

static void
gkm_mate2_private_key_dispose (GObject *obj)
{
	...
	G_OBJECT_CLASS (gkm_mate2_private_key_parent_class)->dispose (obj);
}
...
static void
gkm_mate2_private_key_set_property (GObject *obj, guint prop_id, const GValue *value,
                           GParamSpec *pspec)
{
	switch (prop_id) {
	default:
...
static void
gkm_mate2_private_key_get_property (GObject *obj, guint prop_id, GValue *value,
                           GParamSpec *pspec)
{
	switch (prop_id) {
	default:
...
static void
gkm_mate2_private_key_class_init (GkmMate2PrivateKeyClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	GkmObjectClass *gkm_class = GKM_OBJECT_CLASS (klass);
	GkmSexpKeyClass *key_class = GKM_SEXP_KEY_CLASS (klass);

	gobject_class->dispose = gkm_mate2_private_key_dispose;
	gobject_class->finalize = gkm_mate2_private_key_finalize;
	gobject_class->set_property = gkm_mate2_private_key_set_property;
	gobject_class->get_property = gkm_mate2_private_key_get_property;

	gkm_class->get_attribute = gkm_mate2_private_key_real_get_attribute;

	key_class->acquire_crypto_sexp = gkm_mate2_private_key_real_acquire_crypto_sexp;
}

static gboolean
gkm_mate2_private_key_real_load (GkmSerializable *base, GkmSecret *login, gconstpointer data, gsize n_data)
{
	GkmMate2PrivateKey *self = GKM_MATE2_PRIVATE_KEY (base);
	GkmDataResult res;
...
static void
gkm_mate2_private_key_serializable (GkmSerializableIface *iface)
{
	iface->extension = ".pkcs8";
	iface->load = gkm_mate2_private_key_real_load;
	iface->save = gkm_mate2_private_key_real_save;
}
...
GkmFactory*
gkm_mate2_private_key_get_factory (void)
{
