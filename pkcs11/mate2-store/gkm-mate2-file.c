...
G_DEFINE_TYPE (GkmMate2File, gkm_mate2_file, G_TYPE_OBJECT);
...
static GkmDataResult
update_entries_from_block (GkmMate2File *self, guint section, GHashTable *entries,
                           EggBuffer *buffer, gsize *offset)
{
	...
	g_assert (GKM_IS_MATE2_FILE (self));
	g_assert (entries);
	g_assert (buffer);
	g_assert (offset);
...
static GkmDataResult
update_from_public_block (GkmMate2File *self, EggBuffer *buffer)
{
	gsize offset = 0;
	...
	self->sections |= GKM_MATE2_FILE_SECTION_PUBLIC;

	/* Validate the buffer hash, failure in this case is corruption */
	if (!validate_buffer (buffer, &offset))
		return GKM_DATA_FAILURE;

	return update_entries_from_block (self, GKM_MATE2_FILE_SECTION_PUBLIC,
	                                  self->publics, buffer, &offset);
}

static GkmDataResult
update_from_private_block (GkmMate2File *self, EggBuffer *buffer, GkmSecret *login)
{
	EggBuffer custom;
	GkmDataResult res;
	const gchar *password;
	...
	self->sections |= GKM_MATE2_FILE_SECTION_PRIVATE;
	...
	res = update_entries_from_block (self, GKM_MATE2_FILE_SECTION_PRIVATE,
...
{
	GkmMate2File *self = GKM_MATE2_FILE (data);
	GHashTable *entries;
	guint section;
	...
	if (!gkm_mate2_file_lookup_entry (self, key, &section))
...
static GkmDataResult
update_from_index_block (GkmMate2File *self, EggBuffer *buffer)
{
	gchar *identifier;
	gsize offset;
	guint section;
...
update_from_any_block (guint block, EggBuffer *buffer, GkmSecret *login, gpointer user_data)
{
	...
	g_assert (GKM_IS_MATE2_FILE (user_data));
	self = GKM_MATE2_FILE (user_data);
...
static GkmDataResult
write_entries_to_block (GkmMate2File *self, GHashTable *entries, EggBuffer *buffer)
{
	gsize offset;

	g_assert (GKM_MATE2_FILE (self));
	g_assert (entries);
	g_assert (buffer);
...
static GkmDataResult
write_private_to_block (GkmMate2File *self, EggBuffer *buffer, GkmSecret *login)
{
	EggBuffer secure;
	GkmDataResult res;
...
static GkmDataResult
write_public_to_block (GkmMate2File *self, EggBuffer *buffer)
{
	g_assert (GKM_IS_MATE2_FILE (self));
	g_assert (buffer);
...
static GkmDataResult
write_index_to_block (GkmMate2File *self, EggBuffer *buffer)
{
	g_assert (GKM_IS_MATE2_FILE (self));
	g_assert (buffer);
...
static GkmDataResult
identifier_to_attributes (GkmMate2File *self, const gchar *identifier, GHashTable **attributes)
{
	GHashTable *entries;
	gpointer value;
	guint section;
...
foreach_identifier (gpointer key, gpointer value, gpointer data)
{
	...
	g_assert (GKM_IS_MATE2_FILE (args->self));
...
static void
dump_identifier_and_attributes (GkmMate2File *self, const gchar *identifier, gpointer user_data)
{
	GHashTable *attributes;
	guint section;
	...
	if (!gkm_mate2_file_lookup_entry (self, identifier, &section))
		g_assert_not_reached ();
...
static void
gkm_mate2_file_init (GkmMate2File *self)
{
...
static void
gkm_mate2_file_finalize (GObject *obj)
{
	GkmMate2File *self = GKM_MATE2_FILE (obj);
	...
	G_OBJECT_CLASS (gkm_mate2_file_parent_class)->finalize (obj);
}

static void
gkm_mate2_file_set_property (GObject *obj, guint prop_id, const GValue *value,
                                GParamSpec *pspec)
{
	switch (prop_id) {
	default:
...
static void
gkm_mate2_file_get_property (GObject *obj, guint prop_id, GValue *value,
                                GParamSpec *pspec)
{
	switch (prop_id) {
	default:
...
static void
gkm_mate2_file_class_init (GkmMate2FileClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->finalize = gkm_mate2_file_finalize;
	gobject_class->set_property = gkm_mate2_file_set_property;
	gobject_class->get_property = gkm_mate2_file_get_property;

	signals[ENTRY_ADDED] = g_signal_new ("entry-added", GKM_TYPE_MATE2_FILE,
	                                G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (GkmMate2FileClass, entry_added),
	                                NULL, NULL, g_cclosure_marshal_VOID__STRING,
	                                G_TYPE_NONE, 1, G_TYPE_STRING);

	signals[ENTRY_CHANGED] = g_signal_new ("entry-changed", GKM_TYPE_MATE2_FILE,
	                                G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (GkmMate2FileClass, entry_changed),
	                                NULL, NULL, gkm_marshal_VOID__STRING_ULONG,
	                                G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_ULONG);

	signals[ENTRY_REMOVED] = g_signal_new ("entry-removed", GKM_TYPE_MATE2_FILE,
	                                G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET (GkmMate2FileClass, entry_removed),
	                                NULL, NULL, g_cclosure_marshal_VOID__STRING,
	                                G_TYPE_NONE, 1, G_TYPE_STRING);
}
...
GkmMate2File*
gkm_mate2_file_new (void)
{
	return g_object_new (GKM_TYPE_MATE2_FILE, NULL);
}

GkmDataResult
gkm_mate2_file_read_fd (GkmMate2File *self, int fd, GkmSecret *login)
{
	GkmDataResult res;

	g_return_val_if_fail (GKM_IS_MATE2_FILE (self), GKM_DATA_FAILURE);
	...
	if (res == GKM_DATA_SUCCESS) {
		...
		if (self->privates == NULL && !(self->sections & GKM_MATE2_FILE_SECTION_PRIVATE))
			self->privates = entries_new ();
...
GkmDataResult
gkm_mate2_file_write_fd (GkmMate2File *self, int fd, GkmSecret *login)
{
...
gboolean
gkm_mate2_file_lookup_entry (GkmMate2File *self, const gchar *identifier, guint *section)
{
	gpointer value;

	g_return_val_if_fail (GKM_IS_MATE2_FILE (self), FALSE);
...
void
gkm_mate2_file_foreach_entry (GkmMate2File *self, GkmMate2FileFunc func, gpointer user_data)
{
	ForeachArgs args = { self, func, user_data };

	g_return_if_fail (GKM_IS_MATE2_FILE (self));
...
GkmDataResult
gkm_mate2_file_unique_entry (GkmMate2File *self, gchar **identifier)
{
	gchar *base, *ext;
	guint seed = 1;
	...
	if (*identifier != NULL) {
		if (!gkm_mate2_file_lookup_entry (self, *identifier, NULL))
			return GKM_DATA_SUCCESS;
	}
...
GkmDataResult
gkm_mate2_file_create_entry (GkmMate2File *self, const gchar *identifier, guint section)
{
	GHashTable *attributes;
	GHashTable *entries;
...
GkmDataResult
gkm_mate2_file_destroy_entry (GkmMate2File *self, const gchar *identifier)
{
	GHashTable *entries;
	guint section;
...
GkmDataResult
gkm_mate2_file_write_value (GkmMate2File *self, const gchar *identifier,
...
GkmDataResult
gkm_mate2_file_read_value (GkmMate2File *self, const gchar *identifier,
                          gulong type, gconstpointer *value, gsize *n_value)
{
...
gboolean
gkm_mate2_file_have_section (GkmMate2File *self, guint section)
{
	return (self->sections & section) ? TRUE : FALSE;
}

void
gkm_mate2_file_dump (GkmMate2File *self)
{
	g_print ("PUBLIC:\n\n");
	gkm_mate2_file_foreach_entry (self, dump_identifier_and_attributes,
	                             GUINT_TO_POINTER (GKM_MATE2_FILE_SECTION_PUBLIC));
	g_print ("PRIVATE:\n\n");
	gkm_mate2_file_foreach_entry (self, dump_identifier_and_attributes,
	                             GUINT_TO_POINTER (GKM_MATE2_FILE_SECTION_PRIVATE));
}
